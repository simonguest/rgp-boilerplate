...
let fbAuth = auth.facebook(app, {callbackURL: 'http://localhost:3002/auth/callback', clientID: process.env.FACEBOOK_CLIENT_ID, clientSecret: process.env.FACEBOOK_SECRET});

// Admin GraphQL schema
let adminSchema = buildSchema(require('fs').readFileSync('./graphql/admin.graphqls', 'utf8'));
app.use('/graphql/admin', fbAuth.ensureAuthenticated, graphqlHTTP({schema: adminSchema, rootValue: resolvers.root(pool), graphiql: true}));
